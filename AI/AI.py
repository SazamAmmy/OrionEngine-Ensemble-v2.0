...
"""     
    contents = get_content_from_json_chat(chat_history)
...
def get_content_from_json_chat(json_chat):
    contents = []
    for content in json_chat:
        role = content["role"]
        parts = content["parts"]

        if isinstance(parts, str):
            contents.append(types.Content(role=role, parts=[types.Part.from_text(text=parts)]))

        elif isinstance(parts, bytes):
            print(parts)
            contents.append(types.Content(role=role, parts=[types.Part.from_bytes(data=parts, mime_type="image/png")]))
            
        elif isinstance(parts, list):
            contents.append(types.Content(role=role, parts=[types.Part.from_text(text=part) if isinstance(part, str) 
                                                            else types.Part.from_bytes(data=part, mime_type="image/png") if isinstance(part, bytes) 
                                                            else types.Part.from_text(text="There was some error.") for part in parts]))
    return contents